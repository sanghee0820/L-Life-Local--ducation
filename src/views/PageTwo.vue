                ...
                <template v-for="(infoGroup, groupIndex) in groupedInfo" :key="groupIndex">
                    <div class="info-line">
                        <div v-for="(info, index) in infoGroup" :key="index" class="info">
                            ...
                            <router-link to="/pagethree" class="router-link-three" @click="storeName(info.name)">{{ info.button }}</router-link>
                        </div>
                    </div>
                </template>
                <!-- 페이지네이션 컨트롤 -->
...
export default {
    data() {
        ...
        j : 0,
        allInfo: [
            { name: '', location: '', classTime: '', button : '신청' },     
            { name: '', location: '', classTime: '', button : '신청' },
            { name: '', location: '', classTime: '', button : '신청' },
            { name: '', location: '', classTime: '', button : '신청' },
            { name: '', location: '', classTime: '', button : '신청' },
    ...
    methods: {
        ...
        storeName(name){
            this.$store.commit('setCourse',name);
        }
    }
}
</script>
